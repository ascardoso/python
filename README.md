...
<b>WebViewerEditor.py</b> reads a bookmark.html file, dedupes entries. Selecting one will display its contents. Allows for check validation (404) and removal of those sites, plus using a Json-file to save state (working site or not)

<b>FileViewer.py</b> Asks for a folder to be opened/selected, then lists all files on its left pane, selecting one will display its contents. Supports .txt, .rtf, .rtfd, .png, .jpg and other text and image files
